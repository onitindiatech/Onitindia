...
const PerformerVerificationPage = () => {
    ...
    const handleApprove = async () => {
        ...
        try {
            ...
            setToast({ message: 'Performer identity verified successfully.', type: 'success' });
        } catch (error) {
            setToast({ message: 'Failed to verify performer.', type: 'error' });
        } finally {
    ...
    const handleReject = async () => {
        ...
        try {
            ...
            setRejectReason('');
            setSelectedRequest(null);
            setToast({ message: 'Performer verification rejected.', type: 'success' });
        } catch (error) {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Performer Verification</h1>
                    <p className="text-slate-500 mt-1 font-medium">Verify identity documents and ensure platform trust during onboarding.</p>
                </div>
                <div className="flex items-center gap-3">
                ...
                <div className="card p-0 overflow-hidden flex flex-col h-full sticky top-24">
                    {!selectedRequest ? (
                        <div className="flex-1 flex flex-col items-center justify-center p-12 text-center space-y-4">
                            ...
                            <div>
                                <h3 className="text-lg font-bold text-slate-900">Select a Performer</h3>
                                <p className="text-sm text-slate-400 font-medium">Choose a submission from the list to begin identity verification.</p>
                            </div>
                        </div>
                    ) : (
                                    ...
                                    <div className="aspect-[4/3] bg-slate-900 rounded-xl overflow-hidden relative group">
                                        ...
                                        >
                                            <div className="w-full h-full bg-slate-800 rounded shadow-inner flex flex-col items-center justify-center text-slate-600 gap-2 border-2 border-dashed border-slate-700">
                                                <FileText size={48} />
                                                <span className="text-xs font-bold uppercase tracking-tighter">Identity Proof Preview</span>
                                            </div>
                                        </div>
                                ...
                                <div className="space-y-4">
                                    <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                                        ...
                                        <span className={`px-1.5 py-0.5 rounded text-[8px] ${selectedRequest.ocrScore > 90 ? 'bg-green-100 text-green-600' : 'bg-orange-100 text-orange-600'}`}>
                                            {selectedRequest.ocrScore}% Confidence
                                        </span>
                                    </h4>
                                    <div className="space-y-2">
                                        {[
                                            { label: 'Full Name', val: selectedRequest.name, match: true },
                                            { label: 'Date of Birth', val: '12/05/1984', match: true },
                                            { label: 'Doc Number', val: 'XXXX-XXXX-9821', match: true },
                                        ].map((field, i) => (
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 animate-in zoom-in duration-300">
                        ...
                        <div className="space-y-4">
                            <p className="text-sm text-slate-500 font-medium leading-relaxed">
                                Why are you rejecting <span className="text-slate-900 font-bold">{selectedRequest?.name}'s</span> verification?
                            </p>
...
export default PerformerVerificationPage;
